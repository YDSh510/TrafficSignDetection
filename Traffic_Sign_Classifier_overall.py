...
#file_to_open = data_folder + "test.p"
#print(flenames)
...
def displayData(images, labels, translation):
    ...
    for ax, index in zip(axes.flat, indexes):
        ...
        label = labels[index]
        #print(type(list(translation.keys())[0]))
        #print(type(label))
...
with open('signnames.csv') as f:
    ...
    for row in data:
        #print(row[0],row[1])
        if(row[0] != "ClassId"):
           labelName[int(row[0])] = row[1]
#print(labelName)

displayData(X_train, y_train, labelName)
...
def dataDistribution(ySample,numClass,type):
    
    classList= []
    sampleList=[]
    #print(ySample,numClass)
    for class_n in range(numClass):
        #print(ySample.shape,)
        class_indices = np.where(ySample == class_n)
        #print(class_indices)
        #print(np.shape(class_indices))
        sample_n = len(class_indices[0])
        #print(n_samples)
        classList.append(class_n)
        sampleList.append(sample_n)
    
    #plt.hist(y_train,bins=43)
    plt.bar( classList, sampleList,align='center', alpha=0.5)
    plt.ylabel('Classes')
    plt.xlabel('No of Samples')
    plt.title('Data-Distribution - '+ type)
    plt.show()


    
dataDistribution(y_train,n_classes,"TrainingSamples")
n_classes = len(np.unique(y_valid))
dataDistribution(y_valid,n_classes,"ValidationSamples")
n_classes = len(np.unique(y_test))
dataDistribution(y_test,n_classes,"TestSamples")
...
# X_train = (X_train.astype(np.float64) - 128)/128
# #print(output)

# def normalizingImg(images):
...
#         #print(output.shape,gray.shape,image.shape)
#         #output[ind] = (gray.astype(float) - 128) / 128
#         #print(image)
#         output[ind] = (image.astype(float) - 128) / 128
#     return output

# X_train, y_train = shuffle(X_train, y_train)
# X_train = normalizingImg(X_train)
...
#         img[:,:,1] = gray
#         img[:,:,2] = gray
#         output[ind] = (img.astype(float) - 128) / 128
#         #print(output[ind])
#     return output
...
# X_train, y_train = shuffle(X_train, y_train)
# X_train_t = cv2.cvtColor(X_train.reshape(X_train.shape[0], X_train.shape[1]*X_train.shape[2],X_train.shape[3]), cv2.COLOR_RGB2HSV)
# X_train = (X_train_t.astype(np.float64) - 128)/128
# print(X_train)
...
def TrafficModel(x):    
    ...
    fc4_W = tf.Variable(tf.truncated_normal(shape=(200,  120), mean=mu, stddev=sigma))
...
#     # SOLUTION: Layer 1: Convolutional. Input = 32x32x3. Output = 26x26x32.
...
#     # SOLUTION: Pooling. Input = 26x26x32. Output = 13x13x32.
#     conv1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')

#     # SOLUTION: Layer 2: Convolutional. Input = 13x13x32   Output = 9x9x40.
#     conv2_W = tf.Variable(tf.truncated_normal(shape=(5, 5, 32, 40), mean = mu, stddev = sigma))
...
#     # SOLUTION: Pooling. Input = 9x9x40. Output = 5x5x40.
#     conv2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')

#     # SOLUTION: Flatten. Input = 5x5x40. Output = 640.
#     fc0   = flatten(conv2)
#     print(fc0.shape)
    
# #     fc_W = tf.Variable(tf.truncated_normal(shape=(1000, 800), mean = mu, stddev = sigma))
# #     fc_b = tf.Variable(tf.zeros(800))
# #     fc   = tf.matmul(fc0, fc_W) + fc_b
    
# #     # SOLUTION: Activation.
# #     fc    = tf.nn.relu(fc)
    
#     fc1_W = tf.Variable(tf.truncated_normal(shape=(640, 400), mean = mu, stddev = sigma))
...
#     fc4_W = tf.Variable(tf.truncated_normal(shape=(400,  200), mean=mu, stddev=sigma))
#     fc4_b = tf.Variable(tf.zeros(shape=200))
#     fc4 = tf.matmul(fc1,fc4_W) + fc4_b
...
# val_acc = 
# print(validation_accuracy[0])
# for val in validation_accuracy[0] :
#     val_acc.append(val)
...
imageNames = glob.glob('TrafficSign/*.jp*')
...
for i, img in enumerate(imageNames):
    ...
    print(img, image.shape, resized.shape)
    plt.figure()
    print(plt.xlabel(img))
    plt.imshow(resized)
...
#my_labels = [28,18,25,14,3,9]
my_labels = [3,28,14,25,9,18]
# Check Test Accuracy
...
with tf.Session() as sess:
    saver.restore(sess, tf.train.latest_checkpoint('.'))
    classes = sess.run(logits, feed_dict={x: my_images, keep_prob : 1.0})
    softmax = sess.run(tf.nn.softmax(logits), feed_dict={x: my_images, keep_prob : 1.0})
    print(softmax)

   
with tf.Session() as sess:
    saver.restore(sess, tf.train.latest_checkpoint('.'))
    #print(my_images.shape, my_labels.shape)
    #print(type(my_images),type(my_labels))
    output_accuracy = evaluate(my_images, my_labels)
    print("Testmy_imagesAccuracy = {:.3f}".format(output_accuracy))
